  ...
  const allCombinedTransactions = useMemo(() => {
    ...
    const appLotsAsTxns: TransactionRecord[] = lots.map((lot) => {
      ...
      return {
        ...
        vendorName: lot.facilityName || (lot as any).recyclerName || 'EcoMetals CPCB Dismantling Unit #4',
        ...
        category: (lot.category || 'pcb').toLowerCase(),
        ...
        photoUrl: lot.photoUrl || lot.photos?.topView,
        anomalyFlag: lot.anomalyFlag,
        anomalyReason: lot.anomalyReason,
        isLiveLot: true
      };
    });
                    ...
                    <tr key={tx.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-3 px-4 whitespace-nowrap">
                        <div className="font-bold text-slate-900 flex items-center gap-1.5">
                          <span>{tx.id}</span>
                          {tx.isLiveLot && (
                            <span className="px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-300 text-[9px] font-mono font-bold flex items-center gap-1">
